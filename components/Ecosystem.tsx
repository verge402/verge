  ...
  return (
    <section className="bg-[#F0F0F0] py-20 md:py-32 relative z-20 rounded-b-[20px] md:rounded-b-[24px] lg:rounded-b-[32px] -mb-[20px] md:-mb-[24px] lg:-mb-[32px]">
      <div className="max-w-7xl mx-auto px-3 md:px-5 lg:px-8 text-center">
        <Reveal>
          <h2 className="text-2xl md:text-4xl lg:text-5xl font-light text-[#1B1B1C] mb-3 md:mb-5">
            Building with the{" "}
            <span className="text-emerald-600">best in Base.</span>
          </h2>
          <p className="text-gray-600 text-sm md:text-base mb-14 max-w-[520px] mx-auto">
            Verge integrates with the top Base infrastructure providers.
          </p>
        </Reveal>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-[720px] mx-auto mb-14">
          {partners.map((p, i) => (
            <Reveal key={p.name} delay={100 + i * 60}>
              <div className="bg-white rounded-[20px] p-5 text-center border border-gray-200">
                <span className="text-gray-700 text-[13px] font-medium tracking-[0.04em]">
                  {p.name}
                </span>
              </div>
            </Reveal>
