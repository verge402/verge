 ...
 *
 * Desktop (≥md): 3 nodes in a triangle (Agent / API / Base) connected by
...
const STEPS: Step[] = [
  ...
  { from: "agent",  to: "Base", label: "pay 0.001 USDC",          tone: "magenta", ms: 1100 },
  { from: "Base", to: "agent",  label: "tx 5K4f…3Ax · 400ms",     tone: "violet",  ms: 900  },
...
const NODE_META: Record<NodeKey, { label: string; sub: string; color: string }> = {
  ...
  Base: { label: "Base", sub: "settlement",    color: "var(--color-violet)"  },
};
  ...
  return (
    ...
    >
      ...
      >
        ...
        <line x1={NODES_XY.agent.x}  y1={NODES_XY.agent.y}  x2={NODES_XY.Base.x} y2={NODES_XY.Base.y} stroke="var(--color-line-bright)" strokeOpacity="0.4"  strokeDasharray="3 8" />
        <line x1={NODES_XY.api.x}    y1={NODES_XY.api.y}    x2={NODES_XY.Base.x} y2={NODES_XY.Base.y} stroke="var(--color-line-bright)" strokeOpacity="0.15" strokeDasharray="3 8" />
      ...
      {/* Node cards */}
      {(["agent", "api", "Base"] as NodeKey[]).map((key) => {
...
function MobileFlow({ step, idx, toneColor, isActive }: FlowProps) {
  // Vertical order: AGENT — API — Base
  // Arrow positions: between AGENT↔API (top arrow), between API↔Base (bottom arrow)
  // The middle "AGENT ↔ Base" pair on triangle becomes "skip-the-middle" on mobile.
  // We render label always under the active step.

  const ORDER: NodeKey[] = ["agent", "api", "Base"];
  ...
  return (
    <div className="md:hidden relative mx-auto w-full max-w-[420px]">
      ...
      {/* "Long-arc" indicator for AGENT ↔ Base (when step skips API) */}
      {(step.from === "agent" && step.to === "Base") ||
      (step.from === "Base" && step.to === "agent") ? (
        <div
          key={`longarc-${idx}`}
        ...
        >
          <motion.div
            ...
            style={{
              ...
              transform: step.from === "Base" ? "rotate(180deg)" : "none",
            }}
          >
            {step.label}
          </motion.div>
